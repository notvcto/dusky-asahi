...
has_battery() {
    # Standard path (x86 / most laptops): BAT0, BAT1, ...
    compgen -G "/sys/class/power_supply/BAT*" > /dev/null 2>&1 && return 0
    # Apple Silicon exposes battery as 'macsmc-battery' (no BAT prefix).
    # Read the 'type' file directly — it is always "Battery" for real batteries.
    local type_file
