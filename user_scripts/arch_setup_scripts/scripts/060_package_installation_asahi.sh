#!/usr/bin/env bash
# Asahi Linux / Arch Linux ARM package installation
# -----------------------------------------------------------------------------
# ARM64-adapted version of 060_package_installation.sh for Apple Silicon.
#
# Changes from the x86 version:
#   - Removed: intel-media-driver, vpl-gpu-rt, vulkan-intel, intel-gpu-tools,
#              acpi_call (Intel/NVIDIA-specific, no ARM64 packages)
#   - Removed: linux-headers (use linux-asahi-headers for DKMS on Asahi kernel)
#   - Removed: tlp, tlp-pd, tlp-rdw, thermald (not applicable on Apple Silicon)
#   - Removed: limine, efibootmgr, efitools (Asahi uses its own boot chain)
#   - Added:   power-profiles-daemon (Apple Silicon power management via upower)
#   - Added:   linux-asahi-headers (kernel headers for Asahi kernel)
#   - Note:    mesa-asahi-edge is handled by 051_pacman_asahi_repos.sh
# -----------------------------------------------------------------------------

# Group 1: Graphics & Drivers
declare -ar pkgs_graphics=(
  "mesa" "mesa-utils" "libva" "libva-utils"
  "vulkan-icd-loader" "vulkan-tools"
  "sof-firmware" "linux-firmware"
  # linux-asahi-headers: required for DKMS modules on the Asahi kernel
  "linux-asahi-headers"
  # alsa-ucm-conf-asahi: Apple Silicon audio profiles (speakers, mic, headphone
...
declare -ar pkgs_audio=(
  "pipewire" "pipewire-alsa" "alsa-utils" "wireplumber" "pipewire-pulse"
  "playerctl" "bluez" "bluez-utils" "bluez-hid2hci" "bluez-libs"
  "bluez-obex" "blueman" "bluetui" "pavucontrol"
  "gst-plugins-base" "gst-libav" "gst-plugins-bad" "gst-plugins-good"
  "gst-plugins-ugly" "gst-plugin-pipewire"
  "libcanberra" "songrec" "sox"
)

# Group 6: Filesystem & Archives
declare -ar pkgs_filesystem=(
  "btrfs-progs" "compsize" "zram-generator" "udisks2" "udiskie"
  "dosfstools" "ntfs-3g" "xdg-user-dirs" "usbutils" "gnome-disk-utility"
  "unzip" "zip" "unrar" "7zip" "cpio" "file-roller" "rsync"
  "nfs-utils" "nilfs-utils" "smartmontools" "dmraid" "hdparm"
  "hwdetect" "lsscsi" "sg3_utils" "cpupower" "dust"
  "fcitx5" "fcitx5-gtk" "fcitx5-qt" "dkms"
  ...
  "nemo-image-converter" "nemo-emblems" "nemo-repairer" "nemo-share"
  "python-gobject" "dconf-editor" "xreader" "nemo-pastebin"
)
...
# Group 9: Development
declare -ar pkgs_dev=(
  "neovim" "git" "git-delta" "lazygit" "meson" "cmake" "clang" "uv"
  "rq" "jq" "pv" "bc" "viu" "chafa" "ueberzugpp" "ccache" "mold"
  # shellcheck: not in ALARM aarch64 repos; handled via AUR (shellcheck-bin) in 100_paru_packages_asahi.sh
  "fd" "ripgrep" "fzf" "shfmt" "stylua" "prettier"
  "tree-sitter-cli" "nano" "luarocks"
)

# Group 10: Multimedia
declare -ar pkgs_multimedia=(
  "ffmpeg" "mpv" "mpv-mpris" "satty" "swayimg" "resvg" "imagemagick"
  "libheif" "ffmpegthumbnailer" "grim" "slurp" "wl-clipboard"
  "wl-clip-persist" "cliphist" "tesseract-data-eng"
  # gpu-screen-recorder-ui omitted: requires NVENC or VA-API encode support
  # that is not yet stable on Apple Silicon
)

# Group 11: Sys Admin
# tlp/tlp-pd/tlp-rdw removed: not applicable on Apple Silicon
# thermald removed: Intel thermal daemon, not applicable
# power-profiles-daemon added: works with upower on Apple Silicon
declare -ar pkgs_sysadmin=(
  "btop" "htop" "inxi" "sysstat" "sysbench" "logrotate"
  # nvtop: not available for aarch64 in ALARM repos
  # acpid removed: x86 ACPI event daemon, not applicable on Apple Silicon
  "power-profiles-daemon" "powertop" "gdu" "iotop" "iftop"
  "lshw" "dmidecode" "wev" "pacman-contrib" "gnome-keyring"
  # hwinfo: not available for aarch64 in ALARM repos
  "libsecret" "seahorse" "yad" "dysk" "fwupd" "perl" "accountsservice"
  "smartmontools" "pkgfile" "rebuild-detector"
  # iio-sensor-proxy: ambient light sensor + display auto-brightness on MacBook.
  # Also used by GNOME Night Light and some power-profiles integrations.
  "iio-sensor-proxy"
  # acpid: x86 ACPI event daemon — Apple Silicon does not use ACPI events.
  # Removed (was in x86 list); Apple platform driver handles lid/AC/battery.
)

# Group 12: Gnome Utilities
declare -ar pkgs_gnome=(
  "snapshot" "cameractrls" "loupe" "gnome-text-editor"
  "gnome-calculator" "gnome-clocks"
)
...
ensure_keyring() {
  ...
  print_ok "Arch Linux ARM keyring initialized."
}

refresh_keyring_package() {
  print_info "Refreshing Arch Linux ARM keyring package"
  run_pacman --sync --refresh --needed --noconfirm -- archlinuxarm-keyring
  print_ok "Keyring package is current."
}
